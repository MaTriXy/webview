...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["theme/index"] = factory();
	else
		root["demo"] = root["demo"] || {}, root["demo"]["theme/index"] = factory();
})(this, function() {
...
var Permalinker = function (_InteractivityProvide) {
  _inherits(Permalinker, _InteractivityProvide);

  function Permalinker() {
    _classCallCheck(this, Permalinker);

    return _possibleConstructorReturn(this, (Permalinker.__proto__ || Object.getPrototypeOf(Permalinker)).apply(this, arguments));
  }

  _createClass(Permalinker, [{
    key: 'lazyInit',
    ...
    value: function lazyInit() {
      ...
      this.root_.addEventListener('focusin', function (evt) {
      ...
      this.root_.addEventListener('focusout', function (evt) {
        if (evt.target.classList.contains(classes.COMPONENT_SECTION_PERMALINK)) {
          evt.target.parentElement.classList.remove(classes.COMPONENT_SECTION_HEADING_FOCUS_WITHIN);
        }
      });

      this.window_.addEventListener('hashchange', function () {
        return _this2.flashCurrentSection_();
      });
    ...
    key: 'flashCurrentSection_',
    value: function flashCurrentSection_() {
      var id = location.href.split('#')[1];
      if (!id) {
        return;
      }

      var anchor = this.document_.getElementById(id);
      if (!anchor) {
        return;
      }

      var section = __WEBPACK_IMPORTED_MODULE_2__ponyfill_js__["closest"](anchor, '.' + classes.COMPONENT_SECTION);
      if (!section) {
        return;
      }
    ...
    /** @param {!Document|!Element} root */
    value: function attachTo(root) {
      var instance = new Permalinker(root);
      instance.lazyInit();
      return instance;
    }
  ...
  return Permalinker;
}(__WEBPACK_IMPORTED_MODULE_0__interactivity_js__["b" /* InteractivityProvider */]);

var ThemePicker = function (_InteractivityProvide2) {
  _inherits(ThemePicker, _InteractivityProvide2);
  ...
  _createClass(ThemePicker, [{
     ...
     * @override
     */
    value: function lazyInit(hotSwapper, toolbarProvider) {
      /** @type {!HotSwapper} */
    ...
    key: 'registerThemeMenuOpenHandler_',
    value: function registerThemeMenuOpenHandler_() {
      var menu = new mdc.menu.MDCMenu(this.themeMenuEl_);
      var actionEl = this.root_.getElementById(ids.THEME_COLOR_MENU_ACTION);
    ...
    key: 'registerThemeMenuChangeHandler_',
    value: function registerThemeMenuChangeHandler_() {
      var _this4 = this;

      this.themeMenuEl_.addEventListener(events.MDC_MENU_SELECTED, function (evt) {
        var safeNewTheme = getSafeDemoTheme(evt.detail.item.getAttribute(attributes.THEME_NAME));
        _this4.swapTheme_(safeNewTheme);
        _this4.pushHistoryState_(safeNewTheme);
      });
    }
    ...
    key: 'registerUrlChangeHandler_',
    value: function registerUrlChangeHandler_() {
      var _this5 = this;

      this.window_.addEventListener('popstate', function (evt) {
        var getDefaultState = function getDefaultState() {
          return { theme: getSafeDemoThemeFromUri() };
        };
        var unsafeNewTheme = (evt.state || getDefaultState()).theme;
        var safeNewTheme = getSafeDemoTheme(unsafeNewTheme);
        _this5.swapTheme_(safeNewTheme);
      });
    }
    ...
    key: 'pushHistoryState_',
    value: function pushHistoryState_(safeNewTheme) {
      var hash = this.window_.location.hash || '';
      var url = '?theme=' + (unwrapSafeDemoTheme(safeNewTheme) + hash);
      var data = { theme: unwrapSafeDemoTheme(safeNewTheme) };
    ...
    key: 'swapTheme_',
    value: function swapTheme_(safeNewTheme) {
      var oldLink = this.root_.getElementById(ids.THEME_STYLESHEET);
      var newUri = 'theme-' + unwrapSafeDemoTheme(safeNewTheme) + '.css';
    ...
    key: 'selectCurrentThemeInMenu_',
    value: function selectCurrentThemeInMenu_() {
    ...
    key: 'selectThemeInMenu_',
    value: function selectThemeInMenu_(safeNewTheme) {
      var selectedMenuItemClass = classes.THEME_MENU_ITEM_SELECTED;
      var mdcListItemClass = classes.MDC_LIST_ITEM;
      var themeAttr = attributes.THEME_NAME;
      var menuActionId = ids.THEME_COLOR_MENU_ACTION;

      var unwrappedSafeNewTheme = unwrapSafeDemoTheme(safeNewTheme);
      var oldSelectedItems = __WEBPACK_IMPORTED_MODULE_1__dom_js__["getAll"]('.' + selectedMenuItemClass + '[' + themeAttr + ']', this.themeMenuEl_);
      var newSelectedItem = this.themeMenuEl_.querySelector('.' + mdcListItemClass + '[' + themeAttr + '="' + unwrappedSafeNewTheme + '"]');

      oldSelectedItems.forEach(function (colorIcon) {
        return colorIcon.classList.remove(selectedMenuItemClass);
      });
     ...
     * @param {!ToolbarProvider} toolbarProvider
     */
    value: function attachTo(root, hotSwapper, toolbarProvider) {
      var instance = new ThemePicker(root);
...
demoReady(function (root) {
  Object(__WEBPACK_IMPORTED_MODULE_0__interactivity_js__["d" /* init */])(root);
  Permalinker.attachTo(root);
  ThemePicker.attachTo(root, __WEBPACK_IMPORTED_MODULE_0__interactivity_js__["a" /* HotSwapper */].getInstance(root), __WEBPACK_IMPORTED_MODULE_0__interactivity_js__["c" /* ToolbarProvider */].attachTo(root));
});
